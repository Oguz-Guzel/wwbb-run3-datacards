...
export VERSION=uncalib_noise-combined-2
export DATACARD_DIR_2022=/afs/cern.ch/work/a/aguzel/private/wwbb-run3-datacards/output/uncalib_noise-2022-full/
export DATACARD_DIR_2023=/afs/cern.ch/work/a/aguzel/private/wwbb-run3-datacards/output/uncalib_noise-2023-full/
...
law run PlotUpperLimitsAtPoint --version $VERSION  --multi-datacards $DATACARD_DIR'/output/'$VERSION'/combined_datacard.txt' --datacard-names "Combined" --workers 16

# copy the upper limit plot
output_path=$(law run PlotUpperLimitsAtPoint --version $VERSION  --multi-datacards $DATACARD_DIR'/output/'$VERSION'/combined_datacard.txt' --datacard-names "Combined" --workers 16 --workers 16 --print-out 0 | grep -o 'file://.*' | sed 's|file://||')
...
law run PlotPullsAndImpacts --version $VERSION --datacards $DATACARD_DIR'output/'$BAMBOO_DIR'/combined_2023_datacard.txt',$DATACARD_DIR'output/'$BAMBOO_DIR'/combined_2023BPix_datacard.txt',$DATACARD_DIR'output/'$BAMBOO_DIR'/combined_datacard.txt' --campaign '2023' --PullsAndImpacts-workflow htcondor --order-by-impact

# copy the pulls and impacts plot
output_path=$(law run PlotPullsAndImpacts --version $VERSION --datacards $DATACARD_DIR'output/'$BAMBOO_DIR'/combined_2023_datacard.txt',$DATACARD_DIR'output/'$BAMBOO_DIR'/combined_2023BPix_datacard.txt',$DATACARD_DIR'output/'$BAMBOO_DIR'/combined_datacard.txt' --campaign '2023' --PullsAndImpacts-workflow htcondor --order-by-impact --print-out 0 | grep -o 'file://.*' | sed 's|file://||')

cp "$output_path" $DATACARD_DIR'results/'$BAMBOO_DIR'/2023_pulls_and_impacts.pdf'

# run PlotUpperLimits task (kl scan)
law run PlotUpperLimits --version $VERSION --datacards $DATACARD_DIR'output/'$BAMBOO_DIR'/combined_datacard.txt' --xsec fb --scan-parameters kl,-5,10,31 --y-log --workers 16 --UpperLimits-workflow htcondor --UpperLimits-tasks-per-job 1

output_path=$(law run PlotUpperLimits --version $VERSION --datacards $DATACARD_DIR'output/'$BAMBOO_DIR'/combined_datacard.txt' --xsec fb --scan-parameters kl,-5,10,31 --y-log --workers 16 --UpperLimits-workflow htcondor --UpperLimits-tasks-per-job 1 --print-out 0 | grep -o 'file://.*' | sed 's|file://||')
