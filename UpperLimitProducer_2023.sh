...
export BAMBOO_DIR=v1.4.6-2023-mva
...
law run PlotUpperLimits --version $VERSION --datacards $DATACARD_DIR'output/'$BAMBOO_DIR'/2023_all_combined_datacard.txt' --xsec fb --scan-parameters kl,-20,25,91 --y-log --workers 16 --UpperLimits-workflow htcondor --UpperLimits-tasks-per-job 1 --save-hep-data

output_path=$(law run PlotUpperLimits --version $VERSION --datacards $DATACARD_DIR'output/'$BAMBOO_DIR'/2023_all_combined_datacard.txt' --xsec fb --scan-parameters kl,-20,25,91 --y-log --workers 16 --UpperLimits-workflow htcondor --UpperLimits-tasks-per-job 1 --print-out 0 | grep -o 'file://.*' | sed 's|file://||')
...
output_path=$(law run PlotPostfitSOverB --version $VERSION --datacards $DATACARD_DIR'output/'$BAMBOO_DIR'/DL_boosted_2023.txt',$DATACARD_DIR'output/'$BAMBOO_DIR'/DL_boosted_2023BPix.txt',$DATACARD_DIR'output/'$BAMBOO_DIR'/DL_resolved1b_2023.txt',,$DATACARD_DIR'output/'$BAMBOO_DIR'/DL_resolved1b_2023BPix.txt',$DATACARD_DIR'output/'$BAMBOO_DIR'/DL_resolved2b_2023.txt',$DATACARD_DIR'output/'$BAMBOO_DIR'/DL_resolved2b_2023BPix.txt' --print-out 0 | grep -o 'file://.*' | sed 's|file://||')
...
output_path=$(law run PlotLikelihoodScan  --version $VERSION  --datacards $DATACARD_DIR'output/'$BAMBOO_DIR'/2023_all_combined_datacard.txt' --UpperLimits-custom-args='--X-rtd TMCSO_AdaptivePseudoAsimov=0 --X-rtd TMCSO_PseudoAsimov=0    --X-rt MINIMIZER_freezeDisassociatedParams   --X-rtd MINIMIZER_multiMin_hideConstants --X-rtd MINIMIZER_multiMin_maskConstraints --X-rtd MINIMIZER_multiMin_maskChannels=2  --X-rtd MINIMIZER_skipDiscreteIterations ' --pois kl --scan-parameters kl,-5,10,31 --workers 16 --print-out 0 | grep -o 'file://.*' | sed 's|file://||')
...
law run PlotLikelihoodScan \
    --version $VERSION \
    --datacards $DATACARD_DIR'output/'$BAMBOO_DIR'/2023_all_combined_datacard.txt' \
    --pois kl,kt \
